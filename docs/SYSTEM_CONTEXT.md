...
Use this document as the single source of truth for what the product is, how it's built, and how Watson + the API fit in.
...
**Core loop:** A user talks to an AI named **Watson** about an idea → Watson asks clarifying questions to develop it → the system generates a polished, publication-ready piece of content in one of **12 formats** (LinkedIn Post, Newsletter, Essay, Podcast Script, etc.).
...
| **WORK** | Production layer | 40 AI specialists (simulated via Claude API) transform raw thinking into content, filtered through **Voice DNA** (the user's unique writing style). Watson lives here. |
| **WRAP** | Distribution layer | Packages and schedules content across all channels. |

---

## The AI character: Watson
...
- **Flow:** Once Watson has gathered enough context, the **"Make the thing"** button appears and triggers content generation.
...
2. Watson greets them with an opening question for that format.  
3. User and Watson have a back-and-forth conversation developing the idea.  
4. User clicks **"Make the thing"** → phase shifts to **generating** → Watson orb pulses.  
...
| Area | File |
|------|------|
| Watson UI + chat/generate calls | `src/pages/studio/WorkSession.tsx` |
...
## When changing Watson or the API

- Keep **Watson's voice** as the First Listener: one question at a time, reflect back, use their words.  
