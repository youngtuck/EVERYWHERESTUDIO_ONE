...
**Layer 2: Interest Graph Framework (4 filters)**
...
## LAYER 1: THE 7 MANDATORY QUALITY GATES
...
| 2 | Jordan Lane | >95% Voice DNA match |
| 3 | David Stone | 7-second hook test |
| 4 | Dr. Elena Vasquez | Zero AI padding |
| 5 | Natasha Boyko | Publication-grade + Stranger Test |
| 6 | Dr. Marcus Webb + Marshall | Zero cultural missteps |
...
**Pass criteria:** Category language present, distinctive framing, "I didn't know this existed" reaction
**Fails if:** Sounds like everyone else, good but generic, no category differentiation

### Filter 2: Worldview Marker
**Owner:** Lea (Marketing & Positioning)
**Question:** Does this create curiosity about Mark's broader thinking?
**Pass criteria:** Hints at larger philosophy, reader curious about the person
**Fails if:** Self-contained insight, no thread to pull, expertise without personality

### Filter 3: Conversion Architecture
**Owner:** Guy (Business Development)
**Question:** Is there a natural next step built into the format?
**Pass criteria:** Clear path forward, feels like service not sales, matches content depth
**Fails if:** Great content with no path, generic CTA, disconnected ask

### Filter 4: Qualification Filter
**Owner:** Ward (Sales)
**Question:** Does this attract right-fit prospects and repel wrong-fit?
**Pass criteria:** Clear stance, appropriate complexity, values visible
**Fails if:** Optimized for mass appeal, watered-down positions, attracts tire-kickers

---

## SBU CONVERSION CHECK
...
## BETTERISH INTEGRATION (v5.3)

After the structured SBU evaluation, Betterish delivers the final gut check:

```
SBU Evaluation (Josh -> Lea -> Guy -> Ward -> Fish)
        |
        v
...
**Betterish's Question:** Does this make you want to use it?

---

## QUICK REFERENCE: COMPLETE CHECKPOINT SEQUENCE (v5.4)
...
BETTERISH SCORING (450+ required)
        |
        v
IGF Filter 1: Category Signal (Josh)
        |
        v
IGF Filter 2: Worldview Marker (Lea)
        |
        v
IGF Filter 3: Conversion Architecture (Guy)
        |
        v
IGF Filter 4: Qualification Filter (Ward)
        |
        v
SBU Conversion Check (Guy + Fish)
        |
        v
Betterish: Final Gut Check
        |
...
Format Optimization (Dmitri)
        |
        v
Completeness Verification (Charlie)
        |
        v
DELIVERY
```
...
- Bypass requires Mark's explicit approval
- Documented reason required
- Mandatory AAR to capture learning

### SBU Conversion Check
- Mandatory for all outbound content
- Cannot proceed to FINAL without conversion path confirmed
...
### Interest Graph Framework Failure

**Single Filter Failure:**
1. Strategic Business Unit (SBU) provides specific guidance
2. Targeted revision to address failed filter
3. Re-review on failed filter only

**Multiple Filter Failure (2+ filters):**
1. Strategic Business Unit (SBU) provides comprehensive revision brief
2. Content may need structural rework, not just polish
3. Full re-review required

**Persistent Failure (fails twice):**
1. Escalate to Mark for strategic decision
2. Options: significant rework, repurpose for different channel, or accept as-is with documented exception

---

## VERSION HISTORY

**v5.4 (February 3, 2026)**
- Version bump for consistency
- Encoding cleanup (UTF-8 corruption removed)
...
- Strategic Business Unit (SBU) owns IGF review
- Two-layer quality system documented
- Application matrix by output type
