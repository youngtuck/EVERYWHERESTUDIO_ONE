  ...
  return (
    <div style={{ background:"#0E0E0C", borderRadius:14, padding:"22px", border:"1px solid rgba(255,255,255,0.07)", boxShadow:"0 40px 80px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.02)", width:"100%", maxWidth:440 }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:16, paddingBottom:12, borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
        ...
        <span style={{ fontSize:13, color:"rgba(255,255,255,0.2)", fontFamily:"'Montserrat', sans-serif", letterSpacing:"0.5px" }}>Watson is listening</span>
      </div>
      {/* Input */}
      <div style={{ marginBottom:16, minHeight:56 }}>
        <p style={{ fontSize:14, lineHeight:1.72, color:"rgba(255,255,255,0.78)", fontFamily:"'Montserrat', sans-serif", fontWeight:300 }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:4 }}>
        {FORMATS.map((f,i) => { const on = active && i < cnt; return (
          <div key={f} style={{ padding:"8px 10px", background:on?"rgba(245,198,66,0.07)":"rgba(255,255,255,0.02)", border:`1px solid ${on?"rgba(245,198,66,0.18)":"rgba(255,255,255,0.04)"}`, borderRadius:6, opacity:on?1:0.4, transition:"all 0.18s" }}>
            <p style={{ fontSize:13, fontWeight:on?500:400, color:on?"#F5C642":"rgba(255,255,255,0.38)", fontFamily:"'Montserrat', sans-serif", lineHeight:1.3, transition:"color 0.18s" }}>{f}</p>
          </div>
        );})}
      </div>
      ...
      <div style={{ maxWidth:1160, margin:"0 auto", width:"100%", display:"grid", gridTemplateColumns:"1.1fr 1fr", gap:80, alignItems:"center" }} className="hero-g">
        <div>
          ...
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, marginBottom:32, padding:"5px 12px", background:dark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.04)", border:`1px solid ${dark?"rgba(255,255,255,0.08)":"rgba(0,0,0,0.08)"}`, borderRadius:20 }}>
            ...
            <span style={{ fontSize:14, fontWeight:400, letterSpacing:"0.3px", color:dark?"rgba(255,255,255,0.45)":"rgba(0,0,0,0.42)", fontFamily:"'Montserrat', sans-serif" }}>Composed Intelligence Platform</span>
          </div>

          {/* Headline: very large, serif */}
          <h1 style={{ fontFamily:"'Montserrat', sans-serif", fontSize:"clamp(48px,5.8vw,82px)", fontWeight:400, lineHeight:1.0, letterSpacing:"-1px", color:headC, marginBottom:28 }}>
            Your thinking,<br />
            <em style={{ fontStyle:"italic", color:"#F5C642" }}>composed.</em>
          </h1>

          <p style={{ fontSize:"clamp(15px,1.4vw,17px)", lineHeight:1.82, color:bodyC, maxWidth:420, marginBottom:40, fontFamily:"'Montserrat', sans-serif", fontWeight:300 }}>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:24 }}>
            <button onClick={()=>navigate("/auth")}
              style={{ background:headC, color:bg, border:"none", cursor:"pointer", fontSize:13, fontWeight:500, letterSpacing:"-0.01em", padding:"13px 28px", borderRadius:8, fontFamily:"'Montserrat', sans-serif", transition:"opacity 0.15s" }}
            ...
            <button onClick={()=>navigate("/studio/dashboard")}
              style={{ background:"transparent", border:`1px solid ${secBd}`, color:secC, cursor:"pointer", fontSize:13, fontWeight:400, letterSpacing:"-0.01em", padding:"12px 24px", borderRadius:8, fontFamily:"'Montserrat', sans-serif", transition:"all 0.15s" }}
          ...
          <p style={{ fontSize:14, color:microC, fontFamily:"'Montserrat', sans-serif", fontWeight:300 }}>Invitation only · Founding members onboarding now</p>
        </div>
