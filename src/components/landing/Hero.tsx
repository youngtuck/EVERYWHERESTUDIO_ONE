  ...
  return (
    <div style={{ background:"#0E0E0C", borderRadius:14, padding:"22px", border:"1px solid rgba(255,255,255,0.07)", boxShadow:"0 40px 80px rgba(0,0,0,0.4), 0 0 0 1px rgba(255,255,255,0.02)", width:"100%", maxWidth:440 }}>
      ...
      <div style={{ marginBottom:16, minHeight:56 }}>
        <p style={{ fontSize:14, lineHeight:1.72, color:"rgba(255,255,255,0.78)", fontFamily:"'DM Sans',sans-serif", fontWeight:300 }}>
          {typed}<span style={{ display:"inline-block", width:1.5, height:13, background:"rgba(240,180,41,0.7)", marginLeft:2, verticalAlign:"middle", animation:"blink 1s step-end infinite" }} />
        </p>
      </div>
      {/* Formats */}
...
export default function Hero() {
  ...
  const headC = dark ? "#F0F0EE" : "#111110";
  const bodyC = dark ? "rgba(240,240,238,0.46)" : "rgba(17,17,16,0.5)";
  const microC = dark ? "rgba(240,240,238,0.2)" : "rgba(17,17,16,0.28)";
  ...
  return (
    <section style={{ minHeight:"100vh", display:"flex", alignItems:"center", padding:"90px 32px 80px", background:bg, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ position:"absolute", inset:0, backgroundImage:`radial-gradient(circle at 20% 50%, ${dark?"rgba(58,123,213,0.04)":"rgba(58,123,213,0.05)"} 0%, transparent 50%), radial-gradient(circle at 80% 20%, ${dark?"rgba(240,180,41,0.03)":"rgba(240,180,41,0.06)"} 0%, transparent 50%)`, pointerEvents:"none" }} />
        ...
        <div>
          ...
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", marginBottom:24 }}>
            <button onClick={()=>navigate("/auth")}
              style={{ background:headC, color:bg, border:"none", cursor:"pointer", fontSize:13, fontWeight:500, letterSpacing:"-0.01em", padding:"13px 28px", borderRadius:8, fontFamily:"'DM Sans',sans-serif", transition:"opacity 0.15s" }}
            ...
            <button onClick={()=>navigate("/studio/dashboard")}
              style={{ background:"transparent", border:`1px solid ${secBd}`, color:secC, cursor:"pointer", fontSize:13, fontWeight:400, letterSpacing:"-0.01em", padding:"12px 24px", borderRadius:8, fontFamily:"'DM Sans',sans-serif", transition:"all 0.15s" }}
